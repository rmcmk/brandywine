...
public final class MessageHandlerParser extends TomlParser {
	 ...
	 */
	private static final MessageHandler<Message> DEFAULT_MESSAGE_HANDLER = new DefaultMessageHandler();
	 ...
	 */
	private final Map<Class<? extends Message>, MessageHandler<Message>> handlers = new HashMap<>(MAXIMUM_HANDLERS);
	...
	protected final void append(MessageHandler<Message> handler) {
		...
		Handles annotation = optional.orElseThrow(Suppliers.illegal(StringUtil.simpleClassName(handler) + " is not annotated with @Handles"));
