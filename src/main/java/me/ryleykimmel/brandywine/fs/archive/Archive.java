...
public final class Archive {
	...
	public ArchiveEntry getEntry(String name) {
		return getOptionalEntry(name).orElseThrow(Suppliers.illegal("ArchiveEntry not found for " + name + "."));
	}
	...
	public static Archive decode(Buffer buffer) throws IOException {
		...
		for (int entry = 0; entry < count; entry++) {
			...
			entries.put(identifiers[entry], new ArchiveEntry(identifiers[entry], extracted ? data : CompressionUtil.bunzip2(data)));
		}
