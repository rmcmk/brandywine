...
public final class MovementQueue {
	...
	public void addFirstStep(Position next) {
		...
		/*
		 * We need to connect 'current' and 'next' whilst accounting for the fact that the client and server might be out of sync (i.e. what the client thinks is 'current' is
		 * different to what the server thinks is 'current').
		 *
	...
	public void addStep(Position next) {
		...
		/*
		 * If current equals next, addFirstStep doesn't end up adding anything points queue. This makes peekLast() return null. If it does, the correct behaviour is to fill it in
		 * with mob.getPosition().
		 */
		if (current == null) {
