...
public abstract class Descriptor<T extends Mob, M extends Message> {
	 ...
	 * @param type The UpdateBlocks type.
	 */
	public final void encodeBlock(M message, FrameBuilder blockBuilder, Class<? extends UpdateBlock> type) {
		getBlock(type).ifPresent(block -> updater.getUpdateBlockEncoders().encode(block, message, blockBuilder));
	}
