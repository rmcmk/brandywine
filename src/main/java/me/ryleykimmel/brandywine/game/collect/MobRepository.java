...
public final class MobRepository<T extends Mob> implements Iterable<T> {
	...
	public MobRepository(int capacity) {
		...
		indices = IntStream.range(0, capacity).boxed().collect(Suppliers.collection(new ArrayDeque<>()));
	}
	...
	public T get(int index) {
		...
		if (actual < 0 || actual >= capacity) {
			throw new ArrayIndexOutOfBoundsException("index: " + index + ", actual: " + actual + " is out of bounds, capacity: " + capacity);
		}
