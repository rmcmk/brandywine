...
/**
 * Decodes Frames in a stream-like fashion from one {@link ByteBuf} to another, while maintaining a state.
 *
...
public final class FrameDecoder extends ByteToMessageDecoder {
	...
	private void decodeOpcode(ByteBuf buffer, List<Object> out) {
		...
		opcode = ciphered ? session.decipherFrameOpcode(buffer.readUnsignedByte()) : buffer.readUnsignedByte();
		length = parser.getLength(ciphered, opcode);
		type = parser.getType(ciphered, opcode);

		switch (type) {
		case INVALID:
			logger.warn("The Frame {} is invalid (is it mapped within frames.toml?) and cannot be decoded.", opcode);
			logger.warn("Further Frames cannot be decoded properly for {}, closing session...", session);
			session.close();
			break;
