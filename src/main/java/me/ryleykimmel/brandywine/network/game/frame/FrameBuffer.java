...
abstract class FrameBuffer {
	 ...
	 */
	protected static final int[] BIT_MASKS = { 0x0, 0x1, 0x3, 0x7, 0xf, 0x1f, 0x3f, 0x7f, 0xff, 0x1ff, 0x3ff, 0x7ff, 0xfff, 0x1fff, 0x3fff, 0x7fff, 0xffff, 0x1ffff, 0x3ffff,
	...
	protected final void checkByteAccess() {
		Preconditions.checkArgument(mode == AccessMode.BYTE_ACCESS, "For byte-based calls to work, the mode must be byte access");
	}
	...
	protected final void checkBitAccess() {
		Preconditions.checkArgument(mode == AccessMode.BIT_ACCESS, "For bit-based calls to work, the mode must be bit access");
	}
