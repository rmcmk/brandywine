...
public final class MessageCodecParser extends TomlParser {
  ...
  private static final class DefaultMessageDecoder implements MessageDecoder<Message> {
    ...
    @Override
    public Message decode(GameSession session, FrameReader reader) {
      logger.warn("Frame {} has no attached message decoder", reader.getOpcode());
  ...
  private static final class DefaultMessageEncoder implements MessageEncoder<Message> {
    ...
    public Frame encode(Message message, ByteBufAllocator alloc) {
      logger.error("The {} has no attached MessageEncoder or is not intended to be encoded.",
          message);
   ...
   */
  private static final MessageDecoder<Message> DEFAULT_MESSAGE_DECODER =
  ...
  private <T extends Message> void append(MessageEncoder<T> encoder) {
    ...
    Encodes annotation = optional.orElseThrow(
        Suppliers.illegal(StringUtil.simpleClassName(encoder) + " is not annotated with @Encodes"));
  ...
  private <T extends Message> void append(MessageDecoder<T> decoder) {
    ...
    Decodes annotation = optional.orElseThrow(
        Suppliers.illegal(StringUtil.simpleClassName(decoder) + " is not annotated with @Decodes"));
  ...
  public <T extends Message> Frame encode(T message, ByteBufAllocator alloc) {
    MessageEncoder<T> encoder =
        (MessageEncoder<T>) encoders.getOrDefault(message.getClass(), DEFAULT_MESSAGE_ENCODER);
   ...
   *
   * @param session The GameSession which is decoding the specified Frame.
  ...
  public <T extends Message> T decode(GameSession session, Frame frame) {
    MessageDecoder<T> decoder =
        (MessageDecoder<T>) decoders.getOrDefault(frame.getOpcode(), DEFAULT_MESSAGE_DECODER);
    return decoder.decode(session, new FrameReader(frame));
  }
